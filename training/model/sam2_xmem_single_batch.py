...
class SAM2XMemTrain(SAM2BaseXMem):
    ...
    def forward_tracking_xmem_style(self, backbone_out: dict, input: BatchedVideoDatapoint, data_xmem_style: dict, return_dict=False):
        ...
        for stage_id in processing_order:
            ...
            if img_feats_already_computed:
                ...
                current_vision_feats = [x[:, img_ids] for x in vision_feats] #[0] --> torch.Size([65536, num_objects, 32]), [1] --> torch.Size([16384, num_objects, 64]), [2] --> torch.Size([4096, num_objects, 256]) 
                current_vision_pos_embeds = [x[:, img_ids] for x in vision_pos_embeds]

                C_slots = self.memory_cfg["max_num_objects"]
                needs_padding = any(feat.shape[1] < C_slots for feat in current_vision_feats) #not really necessary to check for any

                if needs_padding:
                    for i in range(len(current_vision_feats)):
                        feat = current_vision_feats[i]          # shape: [N, B, C]
                        pos  = current_vision_pos_embeds[i]
                        b = feat.shape[1]

                        if b < C_slots:
                            needed = C_slots - b
                            pad_feat = feat[:, :1].expand(-1, needed, -1)
                            pad_pos  = pos[:, :1].expand(-1, needed, -1)

                            current_vision_feats[i] = torch.cat([feat, pad_feat], dim=1)
                            current_vision_pos_embeds[i] = torch.cat([pos, pad_pos], dim=1)
    ...
    def make_xmem_input_from_sam2_batch(self, batch: BatchedVideoDatapoint, C: int):
        ...
        - selector: [B, C, 1, 1]
        - num_objects: length-B tensor with counts per video
        """
        # Unpack shapes
        T, B, Cin, H, W = batch.img_batch.shape   # [T, B, 3, H, W]
        # batch.masks: [T, O, H, W] where O = number of annotated slots for this batch
        O = batch.masks.shape[1]

        # import ipdb; ipdb.set_trace()
        # 1) Build rgb
        rgb = batch.img_batch.permute(1, 0, 2, 3, 4).contiguous()  # [B, T, 3, H, W]

        # 2) Build first_frame_gt: [B, 1, C, H, W]
        if O > C:
            raise ValueError(f"Batch contains {O} objects but XMem was configured for C={C} slots.")
        # start with zeros
        first_frame_gt = torch.zeros((B, 1, C, H, W), dtype=torch.float32, device=rgb.device)
        # batch.obj_to_frame_idx[0]: shape [O, 2], rows = (frame_idx=0, video_idx)
        frame0_pairs = batch.obj_to_frame_idx[0]  # Tensor [O, 2]
        # For each object slot k < O, copy mask into channel k of the corresponding video
        for obj_slot in range(O):
            f_idx, b_idx = frame0_pairs[obj_slot].tolist()
            # here f_idx should be 0
            first_frame_gt[b_idx, 0, obj_slot] = batch.masks[0, obj_slot].float()

        # 3) Compute num_filled_objects: how many objects per video at frame 0
        num_filled = [0] * B
        for obj_slot in range(O):
        ...
        # 4) Build selector: [B, C, 1, 1], 1 for active slots < num_filled_objects[b], else 0
        selector = torch.zeros((B, C, 1, 1), dtype=torch.float32, device=rgb.device)
        for b in range(B):
    ...
    def _prompt_decoder_sam(self, 
        ...
        current_out["pred_masks_high_res_xmem"] = high_res_masks

...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        **kwargs,
        ...
        # if xmem_checkpoint_path is not None:
        #     self._load_xmem_weights(xmem_checkpoint_path)
    ...
    def _load_xmem_weights(self, xmem_ckpt_path):
        ...
        def load_sub(name, module, prefix):
            ...
            logging.info(f"[XMem:{name}] Loaded {len(subdict)}/{len(module.state_dict())} params")
    ...
    def forward(self, input: BatchedVideoDatapoint):
        ...
        #data_xmem_style = self.make_xmem_input_from_sam2_batch(input, self.memory_cfg["max_num_objects"])
    ...
    def forward_tracking(
        ...
        for stage_id in processing_order:
            ...
            if img_feats_already_computed:
                ...
                current_vision_feats = [x[:, img_ids] for x in vision_feats] #[0] --> torch.Size([65536, B, 32]), [1] --> torch.Size([16384, B, 64]), [2] --> torch.Size([4096, B, 256]) where B is the batch size
                current_vision_pos_embeds = [x[:, img_ids] for x in vision_pos_embeds]
                #pos_list.append(current_vision_pos_embeds)
            else:
        ...
        if return_dict:
            return output_dict
        # turn `output_dict` into a list for loss functio

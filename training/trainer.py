...
from training.utils_val_metrics import _seg2bmap
...
class Trainer:
    ...
    def _step(
        ...
        #for the loss, we only need "multistep_pred_multimasks_high_res", "multistep_pred_ious" and "multistep_object_score_logits" from the outputs dict 
    ...
    def val_epoch(self, val_loader, phase):
        """
        Validation epoch that:
        - calls model(batch) once per batch,
        - computes true IoU between predicted masks and GT,
        - logs true IoU to TensorBoard.
        """
        ...
        self.model.eval()
        if hasattr(unwrap_ddp_if_wrapped(self.model), "on_validation_epoch_start"):
            unwrap_ddp_if_wrapped(self.model).on_validation_epoch_start()

        for batch in val_loader:
            batch = batch.to(self.device, non_blocking=True)
            with torch.no_grad():
                outputs = self.model(batch)  # List of length B

            gt_bin = (batch.masks > 0).cpu().numpy()  # (B, O, H, W)

            for b, out in enumerate(outputs):
                pred_logits = out["pred_masks_high_res"]  # (O, H, W)
                pred_bin = (pred_logits.sigmoid() > 0.5).cpu().numpy()

                # Validate shape
                B, O, H, W = gt_bin.shape
                bound_pix = np.ceil(0.008 * np.linalg.norm([H, W]))
                ...
                for o in range(O):
                    ...
                    if np.sum(gt) == 0 and np.sum(pred) == 0:
                        ...
                        j = 1.0 if union == 0 else intersection / union
                        
                        # === Boundary F-score ===
                        pred_boundary = _seg2bmap(pred.squeeze(0))
                        gt_boundary = _seg2bmap(gt)

                        pred_dil = cv2.dilate(pred_boundary.astype(np.uint8), boundary_disk)
                        gt_dil = cv2.dilate(gt_boundary.astype(np.uint8), boundary_disk)
                    ...
                    all_j_scores.append(j)
                    all_f_scores.append(f)
        mean_j = np.mean(all_j_scores[1:]) * 100
        mean_f = np.mean(all_f_scores[1:]) * 100
        mean_jf = (mean_j + mean_f) / 2.0

        self.logger.log("Val/J_mean", mean_j, self.steps[Phase.VAL])
        self.logger.log("Val/F_mean", mean_f, self.steps[Phase.VAL])
        self.logger.log("Val/J&F_mean", mean_jf, self.steps[Phase.VAL])

        out_dict = self._log_meters_and_save_best_ckpts([phase])
        out_dict["Val/J_mean"] = mean_j
        out_dict["Val/F_mean"] = mean_f
        out_dict["Val/J&F_mean"] = mean_jf
    ...
    def _run_step(
        ...
        # torch.cuda.synchronize()
        # print(f"Backward pass time: {time.time() - start:.2f} s")
        # import ipdb; ipdb.set_trace()

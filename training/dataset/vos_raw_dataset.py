...
class PNGRawDatasetVal(VOSRawDataset):
    def __init__(
        self,
        img_folder,
        gt_folder,
    ...
    def get_video(self, idx):
        ...
        # ─── Filter out unannotated frames ───
        valid_ids = set(segment_loader.frame_id_to_png_filename.keys())
        frames = [f for f in frames if f.frame_idx in valid_ids]
        if len(frames) == 0:
            raise RuntimeError(f"No annotated frames for video {video_name}")

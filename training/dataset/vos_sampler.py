...
class RandomUniformSamplerDilat(VOSSampler):
    def __init__(
        self,
        num_frames,
        max_num_objects,
        reverse_time_prob=0.0,
        temporal_dilation=1,  # 👈 New argument
    ):
        self.num_frames = num_frames
        self.max_num_objects = max_num_objects
        self.reverse_time_prob = reverse_time_prob
    ...
    def sample(self, video, segment_loader, epoch=None):
        required_video_len = self.temporal_dilation * (self.num_frames - 1) + 1

        for retry in range(MAX_RETRIES):
            if len(video.frames) < required_video_len:
                raise Exception(
                    f"Cannot sample {self.num_frames} frames with dilation={self.temporal_dilation} "
                    f"from video {video.video_name} (only {len(video.frames)} frames)."
                )

            max_start = len(video.frames) - self.temporal_dilation * (self.num_frames - 1)
            start = random.randrange(0, max_start)
            frames = [
                video.frames[start + step * self.temporal_dilation]

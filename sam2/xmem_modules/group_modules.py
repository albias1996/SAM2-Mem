...
class SepGConv2D(nn.Module):
    """
    Depthwise-separable version of GConv2D that supports grouped tensors
    shaped [B, O, C, H, W] just like GConv2D.
    """
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, padding=1, bias=True):
        super().__init__()
        # Depthwise: per-channel spatial conv; groups=in_ch
        self.depthwise = GConv2D(
            in_ch, in_ch,
    ...
    def forward(self, g):
        # GConv2D already handles [B,O,C,H,W] by flattening (B,O)
        g = self.depthwise(g)
        g = self.pointwise(g)
        return g

class SepMainConv2d(nn.Module):
...
class GroupResBlockSep(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.downsample = None if in_dim == out_dim else SepGConv2D(in_dim, out_dim, 3, padding=1)
        self.conv1 = SepGConv2D(in_dim, out_dim, 3, padding=1)
        self.conv2 = SepGConv2D(out_dim, out_dim, 3, padding=1)

    def forward(self, g):
        out = self.conv1(F.relu(g))

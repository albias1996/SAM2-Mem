...
from sam2.xmem_modules.kv_memory_store import KeyValueMemoryStore
...
class MemoryManager:
    ...
    def add_memory(self, key, shrinkage, value, objects, selection=None):
        ...
        if self.H is None or self.reset_config:
            ...
            #1) this define the min and max number of elements allowed in the working memory and is based on frame counts x spatial resolution
            if self.enable_long_term:
        ...
        #2) flatten all spatial dimensions 
    ...
    def create_hidden_state(self, n, sample_key):
        ...
        if self.hidden is None:
            #This initializes the sensory memory h_0 to all zeros (for all objects).
            self.hidden = torch.zeros((1, n, self.hidden_dim, h, w), device=sample_key.device)
        elif self.hidden.shape[1] != n:
            #If new objects are introduced, or the hidden state is missing/short
            #so the hidden state is - Expanded to handle new objects and - Their initial memory is zero
    ...
    def set_hidden(self, hidden):
        #it updated the current GRU hidden state, which is stored in self.hidden
        self.hidden = hidden

    def get_hidden(self):
        #it just returns the current GRU hidden state, which is stored in self.hidden (note this is None for the first frame)
        return self.hidden

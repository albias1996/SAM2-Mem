...
class KeyValueMemoryStore:
    ...
    def add(self, key, value, shrinkage, selection, objects: List[int]):
        #1) create new_count and new_life for the new keys
        ...
        #2) add the triplet (k, s, e) to the memory
        # add the key
        if self.k is None:
            self.k = key
            self.s = shrinkage
        ...
        #3) handle the v values per object group 
        # add the value
        if objects is not None:
            ...
            #4) assign values to existing object groups
    ...
    def remove_obsolete_features(self, max_size: int):
        ...
        #1) compute usage score --> it represents how frequently each memory element is actually used when the attention weights are computed
        usage = self.get_usage().flatten()

        #2) identify least used entries --> The ones not in that list will be kept (survived = True)
        values, _ = torch.topk(usage, k=(self.size-max_size), largest=False, sorted=True)
        survived = (usage > values[-1])

        #3) remove the least used entries
        ...
        #4) handle values per object group --> note: LT memory cleanup only works with 1 group

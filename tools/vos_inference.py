...
from sam2.build_sam import build_sam2_video_predictor, build_sam2_xmem_video_predictor
...
def vos_inference(
    ...
    # Make sure GPU is empty before timing
    ...
    total_time_s = start_evt.elapsed_time(end_evt) / 1000.0  # convert to seconds

    fps = total_frames / total_time_s 
    # print(f"[{video_name}] processed {total_frames} frames in {total_time_s:.3f}s → FPS: {fps:.1f}")
    
    ##print height and width
    # print(f"[{video_name}]: ({height}x{width})")
...
def vos_inference_xmem_style(
    ...
    for input_frame_idx in input_frame_inds:
        ...
        frame_idx, obj_ids, video_res_masks = predictor.add_new_mask_xmem_style(
            inference_state=inference_state,
            frame_idx=input_frame_idx,
            object_ids=object_ids_set,
            per_obj_input_mask=per_obj_input_mask,
        )
        per_obj_output_mask = {
            out_obj_id: (video_res_masks[i] > 0.5).cpu().numpy()
    ...
    output_palette = input_palette or DAVIS_PALETTE
    #for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(
    for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video_xmem_style(
        inference_state #shape of out_mask_logits is torch.Size([2, 1, 1280, 720])
    ):  
        per_obj_output_mask = {
            out_obj_id: (out_mask_logits[i] > score_thresh).cpu().numpy() #score_thresh
            for i, out_obj_id in enumerate(out_obj_ids)
        }
        video_segments[out_frame_idx] = per_obj_output_mask
...
@torch.inference_mode()
@torch.autocast(device_type="cuda", dtype=torch.bfloat16)
def vos_inference_xmem_style_later_object(
    predictor,
    base_video_dir,
    input_mask_dir,
    output_mask_dir,
    ...
    for object_id in object_ids:
        ...
        for input_frame_idx in input_frame_inds:
            out_frame_idx, _, video_res_masks = predictor.add_new_mask_xmem_style_single_object(
                inference_state=inference_state,
                frame_idx=input_frame_idx,
                input_mask=inputs_per_object[object_id][input_frame_idx],
                object_id=object_id,
            )
            # import ipdb; ipdb.set_trace()
            obj_scores = video_res_masks.cpu().numpy() 
            output_scores_per_object[object_id][out_frame_idx] = obj_scores

        # run propagation throughout the video and collect the results in a dict
        for out_frame_idx, _, out_mask_logits in predictor.propagate_in_video_xmem_style(
            inference_state,
            start_frame_idx=min(input_frame_inds)+1,  # start from the next frame after the input
            reverse=False,
        ):
...
def main():
    ...
    parser.add_argument(
        "--xmem_style",
        action="store_true",
        help="whether to use xmem style inference (i.e. separate inference per object) "
    )
    parser.add_argument(
        "--xmem_ckpt_path",
        default=None,
        type=str,
        help="path to the XMem model checkpoint, this one is not used anymore because it supposed sam2 and xmem weights are in separated pth.",
    )

    # parser.add_argument(
    #     "--light_verion_sam2_mem",
    #     action="store_true",
    #     help="choose between the original value encoder or the light one for the sam2-mem model",
    # )
    ...
    if args.xmem_style:
        predictor = build_sam2_xmem_video_predictor(
    ...
    for n_video, video_name in enumerate(video_names):
        ...
        if not args.track_object_appearing_later_in_video:
            ...
            if args.xmem_style:
                ...
                fps_list.append(fps)
                allocated_memory_mb_list.append(torch.cuda.max_memory_allocated() / (2**20))
            else:
                fps = vos_inference(
                    predictor=predictor,
                    base_video_dir=args.base_video_dir,
            ...
            if args.xmem_style:
                fps = vos_inference_xmem_style_later_object(
                    predictor=predictor,
                    base_video_dir=args.base_video_dir,
                    input_mask_dir=args.input_mask_dir,
                    output_mask_dir=args.output_mask_dir,
                ...
                fps = vos_separate_inference_per_object(
                    predictor=predictor,
                    base_video_dir=args.base_video_dir,
                    input_mask_dir=args.input_mask_dir,
                    output_mask_dir=args.output_mask_dir,
    ...
    # mean_memory_mb = sum(allocated_memory_mb_list) / len(allocated_memory_mb_list)
    mean_fps = sum(fps_list) / len(fps_list)
    print(
        #f"Mean allocated memory over {len(video_names)} videos: {mean_memory_mb:.1f} MB \n"
        f"Mean FPS over {len(video_names)} videos: {mean_fps:.1f} \n"
